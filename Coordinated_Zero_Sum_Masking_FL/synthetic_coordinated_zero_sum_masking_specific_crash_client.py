"""Dataset-I CZSM client with simulated fit/evaluation failure support."""
...
class FlowerClient(NumPyClient):
    ...
    def _maybe_crash(self, config, phase: str) -> None:
        ...
        if "crash_on_round" in config and "crash_phase" in config:
            crash_round = int(config["crash_on_round"])
            crash_phase = str(config["crash_phase"])
            if server_round == crash_round and phase == crash_phase:
                print(
                    f"[Client {self.partition_id}] simulated {phase} "
                    f"failure at round {server_round}"
    ...
    def fit(self, parameters, config):
        # The assignment is generated before this call. A failure here therefore
        # represents dropout after mask assignment, for which exact cancellation
        # is not guaranteed unless the remaining masks are regenerated.
        ...
        if "masking_vector" in config:
            try:
                ...
                print(
                    f"[Client {self.partition_id}] invalid masking "
                    f"assignment: {exc}"
                )
...
if __name__ == "__main__":
    ...
    train_data, test_data = load_partition(
        args.partition_id, args.num_partitions
    )
    client = FlowerClient(
        Net(), train_data, test_data, args.partition_id
    )
